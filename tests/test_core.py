...
class TestDatabase:
    ...
    def test_register_and_retrieve_by_discord_id(self):
        db = self._make_db()
        db.register(discord_id=123, nation_id=456)
    ...
    def test_register_updates_existing_entry(self):
        db = self._make_db()
        db.register(discord_id=123, nation_id=456)
        db.register(discord_id=123, nation_id=789)
        row = db.get_by_discord_id(123)
    ...
    def test_nation_id_unique_across_users(self):
        db = self._make_db()
        db.register(discord_id=111, nation_id=456)
        with pytest.raises(sqlite3.IntegrityError):
            with db._connect() as conn:
                conn.execute(
                    "INSERT INTO registrations (discord_id, nation_id, registered_at) "
                    "VALUES (?, ?, ?)",
                    ("222", 456, "2024-01-01T00:00:00+00:00"),
                )
